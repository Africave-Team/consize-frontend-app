  ...
  return (
    <Menu>
      ...
      <MenuList className='text-sm' minWidth={'200px'}>
        ...
        {!single && <MenuItem onClick={() => router.push(`/dashboard/courses/${course.id}/modify`)} className='hover:bg-gray-100' icon={<FiEdit2 className='text-sm' />}>Edit {course.bundle ? 'bundle' : 'course'}</MenuItem>}
        {single && <MenuItem onClick={() => router.push(`/dashboard/courses/${course.id}/modify/${course.status === CourseStatus.DRAFT && course.source === Sources.MANUAL ? 'course-info' : 'outline'}`)} className='hover:bg-gray-100' icon={<FiEdit2 className='text-sm' />}>Edit {course.bundle ? 'bundle' : 'course'} in builder</MenuItem>}
        {course.status !== CourseStatus.DRAFT && <MenuItem onClick={() => router.push(`/dashboard/courses/${course.id}/modify/settings`)} className='hover:bg-gray-100' icon={<FiSettings className='text-sm' />}>{course.bundle ? 'Bundle' : 'Course'} settings</MenuItem>}
