          ...
          {tab === 'link' && <>
            ...
            {selectedCohort && <>
              ...
              <div className='mb-4 flex justify-between gap-1'>
                <Input type="text" className="bg-gray-200 text-gray-600 !opacity-90 cursor-not-allowed px-2 py-1 border rounded-md overflow-y-auto w-11/12" disabled value={`https://${team?.domains && team.domains.length < 2 ? team.domains[0].host : team?.domains[1].host}/courses/${isBundle ? `bundles/` : ''}${course.id}?cohort=${selectedCohort.id}`} id="link-content" />
