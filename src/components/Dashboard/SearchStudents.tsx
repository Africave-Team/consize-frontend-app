...
export default function SearchStudents ({ students, courseId }: { students: RTDBStudent[], courseId: string }) {
  const { isOpen, onClose, onOpen } = useDisclosure()
  const [search, setSearch] = useState("")
  const [searchResults, setSearchResults] = useState<RTDBStudent[]>([])
  const router = useRouter()

  const handleSearch = (key: string) => {
    const results = students.filter((item: RTDBStudent) => JSON.stringify(item).toLowerCase().indexOf(search.toLowerCase()) != -1)
            ...
            {searchResults && <div>
              {searchResults.length === 0 ? <div className='w-full h-10 flex justify-center items-center mt-4'>
                ...
                <div className='w-full overflow-y-scroll'>
                  {searchResults.map((student: RTDBStudent) => (<div onClick={() => router.push(`/dashboard/courses/${courseId}/enrollments/${student.id}`)} className='min-h-10 cursor-pointer hover:bg-[#0D1F23] hover:text-white p-2' key={student.phoneNumber}>
                    <div className='text-sm font-bold'>{student.name} {'->'} {student.phoneNumber}</div>
                  </div>))}
                </div>
              </div>}
            </div>}
