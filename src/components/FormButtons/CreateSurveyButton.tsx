...
import { v4 } from 'uuid'
import { createSurveyRequest } from '@/services/survey.services'
...
export default function CreateSurveyButton ({ onFinish }: { onFinish: (createdSurveyId?: string) => void }) {
  ...
  return (
    <>
      <button onClick={onOpen} className='flex gap-1 font-medium items-center justify-center px-8 rounded-2xl border h-10'>
        <FiPlus />
        Create survey
      </button>
      ...
      >
        ...
        <ModalContent className='min-h-48 p-0'>
          <ModalBody className='h-48 px-5 py-5'>
            <h2 className='font-semibold text-xl'>Create a survey</h2>
              ...
              <div className='mt-5 justify-between flex items-center'>
                ...
                <div className='flex gap-2 items-center'>
                  ...
                  <button disabled={!form.isValid || form.isSubmitting} type='submit' className='h-12 w-40 flex gap-2 disabled:bg-primary-dark/50 justify-center items-center bg-primary-dark text-white rounded-md border'>
                    Save survey {form.isSubmitting && <Spinner size={'xs'} />}
                  </button>
                </div>
              </div>
            </form>
