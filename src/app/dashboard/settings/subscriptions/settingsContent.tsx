      ...
      <div className='w-full overflow-y-scroll max-h-full p-4'>
        <div className='flex justify-center items-center gap-5 md:flex-row flex-col'>
          {plans && plans.map((plan) => (
            <div key={plan.id} className={`h-96 group w-72 border rounded-lg flex flex-col justify-between hover:text-white hover:bg-primary-dark p-4 ${subscription && subscription.plan === plan.id ? 'bg-primary-dark text-white' : ''}`}>
